        ...
        try:
            ...
            with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
                ...
                for ch in channels_to_process:
                    # Ambil nama kategori / group title, pertahankan huruf aslinya
                    group = ch.get("group", ch.get("category", ch.get("category_name", ch.get("_auto_group", ""))))
                    
                    # FILTER UTAMA: Hanya proses jika kategori mengandung kata "series" (tidak case-sensitive)
                    ...
                    stream_url = ""
                    if ch_id:
                        # Menggunakan HLS standar yang terbukti sukses dan lancar untuk series
                        stream_url = f"https://boti.my.id/index.m3u8?u={u}&x={x}&a={a}&id={ch_id}&type=hls&api=video"
                    elif "url" in ch:
                        stream_url = ch["url"]
                    elif "link" in ch:
                        stream_url = ch["link"]
            ...
            print(f"Sukses! {count} item dari kategori Series berhasil di-generate.")
